...
export function HQLeadersClient({ leaders, potentialLeaders, viewerRole, facultySlug, facultyId }: HQLeadersClientProps) {
  ...
  const handleAssignLeader = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Upsert profile with leadership_role (Users keep their original role)
                ...
                >
                  ...
                  {potentialLeaders.map(p => (
                    <option key={p.id} value={p.id}>
                      {p.fullName} ({p.email}) — Current: {p.currentRole === "COORDINATOR" ? "Zonal Leader" : p.currentRole === "ADMIN" ? "Admin" : "Student"}
                    </option>
                  ))}
                </select>
              </div>
